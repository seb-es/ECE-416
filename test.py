...
async def send_angles(angles):
    try:
        async with websockets.connect(SERVER_URI) as websocket:
            data = {"servo_angles": [int(np.degrees(angle)) for angle in angles]}
...
def main():
        ...
        t = np.array([[0,0,0], [a2,0,a1], [a3,0,0]])  # Joint translations
        
        p_eff_2 = [0,0,a4]  # End effector position
        k_c = RoboticArm(k,t)
            ...
            if len(lmsList[0]) != 0:
                ...
                if frame_count % 2 == 0:
                    ...
                    endeffector_goal_position = np.array([
                        3 + displacement[0]/400,  # Scale down displacement 
                        0 + displacement[1]/200,
                        5 + displacement[2]/800
                    ])
                    final_q = k_c.pseudo_inverse(q_current, p_eff_N=p_eff_2, goal_position=endeffector_goal_position, max_steps=200)
