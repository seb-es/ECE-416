...
def main():
    ...
    while True:
        ...
        if len(lmsList[0]) != 0:
            ...
            elif forwardTilt <= -0.3 and servo5_angle < 160:
            ...
            if sidewaysTilt >= 0.3 and servo4_angle > 0:
                servo4_angle = max(0, servo4_angle - 2)  # Decrement but don't go below 20
            elif sidewaysTilt <= -0.3 and servo4_angle < 180:
        ...
        if cv2.waitKey(1) == ord('x'):
            angles_radians = [np.radians(0), np.radians(180), np.radians(-150)]
            asyncio.run(send_angles(angles_radians, False, 90, 90))
            break
