...
def main():
  ...
  # All joints are assumed to be revolute joints, and rotate about the z-axis
  # k = kx, ky, kz 
  k = np.array([[0,0,1],    # 1st Joint Rotation Axis
                [0,0,1],    # 2nd Joint Rotation Axis
                [0,0,1],    # 3rd Joint Rotation Axis
                [0,0,1],    # 4th Joint Rotation Axis
                [0,0,1]])   # 5th Joint Rotation Axis
  ...
  final_q = k_c.pseudo_inverse(q_0, p_eff_N=p_eff_2, goal_position=endeffector_goal_position, max_steps=750)
     
  # Final Joint Angles in degrees   
  print('\n\nFinal Joint Angles in Degrees')
  print(f'Joint 1: {np.degrees(final_q[0])} , Joint 2: {np.degrees(final_q[1])}, Joint 3: {np.degrees(final_q[2])}, Joint 4: {np.degrees(final_q[3])}, Joint 5: {np.degrees(final_q[4])}')
